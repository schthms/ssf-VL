...
class ViLBERT(BaseModel):
    ...
    def build(self):
        ...
        if getattr(self.config, "freeze_text", False):
            print("Freezing text encoding layers!")
            c = 0
            for p in self.model.bert.encoder.layer.parameters():
                p.requires_grad = False
                c += 1
            print(f"freezed {c} layers out of...")
            c = 0
            for p in self.model.bert.parameters():
                c += 1
            print(c)
