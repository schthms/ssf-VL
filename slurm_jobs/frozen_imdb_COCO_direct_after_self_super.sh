...
# Run me with (update name!)   sbatch --gpus-per-node=4 frozen_imdb_COCO_direct_after_self_super.sh

# Set environment variables
echo "Setting environment variables..."
export RUN_NAME="frozen_imdb_COCO_direct_after_pretrain"
...
unzip -qn mmimdb.zip && unzip -qo mmimdb_self_super.zip
...
# Start from pretrained with checkpoint.resume=True \
...
mmf_run config=projects/visual_bert/configs/mmimdb/defaults.yaml \
    run_type=test \
    dataset=mmimdb \
    model=visual_bert \
    training.fp16=True \
