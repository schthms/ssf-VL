...
# Run me with (update name!)   sbatch --gpus-per-node=4 vilBERT_twitter_099_001_direct_after_self_super.sh

# Set environment variables
echo "Setting environment variables..."
export DATA="twitter"
export RUN_NAME=$DATA"_vilBERT_099_001_direct_after_self_super"
...
# Run
mmf_run config=projects/vilbert/configs/"$DATA"/defaults.yaml \
    run_type=train_val \
    dataset="$DATA" \
    model=vilbert \
    training.fp16=True \
