...
# Run me with (update name!)   sbatch --gpus-per-node=4 twitter_COCO_direct_after_self_super.sh

# Set environment variables
echo "Setting environment variables..."
export RUN_NAME="twitter_COCO_direct_after_self_super_20k"
...
unzip -qn twitter.zip && unzip -qo twitter_self_super.zip

# !!! CHOSE ONE !!!
# option 1: one of the label splits labels.jsonl or no_labels.jsonl
mv twitter_self_super/labels.jsonl twitter/defaults/annotations/train.jsonl
# option 2: original full train.jsonl
#mv twitter_full_train.jsonl twitter/defaults/annotations/train.jsonl
...
# Start from pretrained with checkpoint.resume_pretrained=True
#    checkpoint.resume_file="$TMPDIR"/thomass_scratch/save/pretrained.ckpt
mmf_run config=projects/visual_bert/configs/twitter/defaults.yaml \
    run_type=test \
    dataset=twitter \
    model=visual_bert \
    training.fp16=True \
