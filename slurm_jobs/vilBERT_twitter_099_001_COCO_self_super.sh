...
# Run me with (update name!)   sbatch --gpus-per-node=4 vilBERT_twitter_099_001_COCO_self_super.sh

# Set environment variables
echo "Setting environment variables..."
export RUN_NAME="twitter_vilBERT_099_001_COCO_self_super_60k"
...
unzip -qn twitter.zip && unzip -qo twitter_split_099_001.zip

# !!! CHOSE ONE !!!
# option 1: one of the label splits labels.jsonl or no_labels.jsonl
mv twitter_split_099_001/no_labels.jsonl twitter/defaults/annotations/train.jsonl
...
mmf_run config=projects/vilbert/configs/twitter/pretrain.yaml \
    run_type=train \
    dataset=masked_twitter \
    model=vilbert \
    training.fp16=True \
    ...
    training.max_updates=60000 \
    scheduler.params.num_training_steps=60000 \
