...
# Run me with (update name!)   sbatch --gpus-per-node=4 food_COCO_full_direct.sh

# Set environment variables
echo "Setting environment variables..."
export RUN_NAME="food_COCO_full_direct_20k"
...
unzip -qn food101.zip && unzip -qo food101_self_super.zip
...
mmf_run config=projects/visual_bert/configs/food101/defaults.yaml \
    run_type=test \
    dataset=food101 \
    model=visual_bert \
    training.fp16=True \
