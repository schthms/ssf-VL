...
# Run me with (update name!)   sbatch --gpus-per-node=4 twitter_COCO_less_direct.sh

# Set environment variables
echo "Setting environment variables..."
export DATA="twitter"
export RUN_NAME=$DATA"_COCO_less_direct_20k"
...
unzip -qn "$DATA".zip && unzip -qo "$DATA"_self_super.zip

# !!! CHOSE ONE !!!
# option 1: one of the label splits labels.jsonl or no_labels.jsonl
mv "$DATA"_self_super/labels.jsonl "$DATA"/defaults/annotations/train.jsonl
